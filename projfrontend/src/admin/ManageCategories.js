    ...
    <Base title="Welcome admin" description="Manage products here">
      ...
      <div className="row">
        <div className="col-12">
          <h2 className="text-center text-white my-3">{categories.length > 0 ? `Total ${categories.length} products` : "No product to show"} </h2>
          {
            categories.length > 0 && 
            categories.map((category) => (
                <div key={category._id} className="row text-center mb-2 ">
                  ...
                  <div className="col-4">
                    <button onClick={() => {deleteMyProduct(category._id)}} className="btn btn-danger">
                      Delete
                    </button>
                  </div>
                </div>
